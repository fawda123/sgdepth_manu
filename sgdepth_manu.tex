...
\author{
  ...
  \vspace{1in} 
  \\ Version Date:   Wed Mar 22 10:09:07 2017 -0500
	}
\date{}
\maketitle
\end{singlespace}
...
\caption{Uncorrected satellite estimates of light attenuation in Choctawhatchee Bay (a) and example of correction with \textit{in situ} data (b). \textit{In situ} data of light attenuation were estimated as an annual average (2010) for monthly data at the sampling sites labelled as points in (a) (mean depth 7.2 m, minimum 3.5 m).  The corresponding satellite data in the same grid cells were compared to the \textit{in situ} data based on regressions of each dataset with cumulative estimates for both.  An example correction is shown in (b) where for any uncorrected satellite estimate (point 1), the corresponding cumulative estimate on the regression curve was identified (point 2), matched with the same cumulative estimate from the \textit{in situ} data (point 3), and then matched to the corresponding value on the x-axis (point 4) to yield the corrected satellite estimate.}
\label{fig:satcorr}
\end{figure}
