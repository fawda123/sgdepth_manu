...
The uncertainty associated with the \ac{doc} estimates were based on the upper and lower bounds for the fitted logistic curve.  Specifically, a linear curve was fit through the upper and lower logistic curves to estimate $SG_{max}$, $DOC_{med}$, and $DOC_{maxd}$.  These values were compared with the initial estimates from the linear curve that was fit through the predicted logistic curve (i.e., \cref{fig:est_ex3}).  Although this approach provides an indication of uncertainty for individual estimates, the analysis can be repeated  for differnt radii to estimate how uncertainty varies with sampling area.    
