...
rows <- tab[, 2]
tab <- tab[, -c(1:2)]
tab[, 2:5] <- formtab_fun(tab[, 2:5])
tab[tab == ' NaN'] <-  ' '

cap.val<-'Summary of maximum depth of colonization ($Z_{c,\\,max}$, m) for all bay segments of Choctawhatchee Bay, Indian River Lagoon, and Tampa Bay.  See \\cref{fig:light_choc,fig:light_tb,fig:light_irl} for spatial distribution of the results.'
...
col_heads <- c('$n$', 'Mean', 'St. Dev.', 'Min', 'Max')
...
# prep for table
tab <- data.frame(all_light)
tab <- tab[, !grepl('_z_cmax$', names(tab))]
rows <- tab[, 2]
tab <- tab[, -c(1:2)]
tab[, 2:5] <- formtab_fun(tab[, 2:5])
tab[tab == ' NaN'] <-  ' '

cap.val<-'Summary of estimated light requirements as percentage of surface irradiance for all bay segments of Choctawhatchee Bay, Indian River Lagoon, and Tampa Bay.  See \\cref{fig:light_choc,fig:light_tb,fig:light_irl} for spatial distribution of the results.'
...
grid.arrange(
  ...
  ncol = 1, heights = c(0.2, 1,1,1,1.25)
)
dev.off()
