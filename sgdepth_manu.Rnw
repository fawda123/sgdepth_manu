...
cap.val <- "Summary of uncertainty for seagrass depth estimates (m) for each segment using all grid locations in \\cref{fig:all_ests}.  Summaries are based on the widths of 95\\% prediction intervals.  The uncertainty values are equally applicable to each seagrass depth measure ($Z_{c,\\,min}$, $Z_{c,\\,med}$, $Z_{c,\\,max}$)."
...
grid.arrange(
arrangeGrob(grid::textGrob(lab1), grid::textGrob(lab2), ncol = 2), #widths = sum(grobwidths)/2), 
