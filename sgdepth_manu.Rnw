...
all_light <- rbind(choc_light, irl_light, tb_light) %>% 
  ...
  summarize(
    n_locs = length(seg), 
    mean_z_cmed = mean(z_c_all, na.rm = T), 
    se_z_cmed = sd(z_c_all, na.rm = T),
    min_z_cmed = min(z_c_all, na.rm = T),
    max_z_cmed = max(z_c_all, na.rm = T),
    n_locsl = length(na.omit(light)),
    mean_light = mean(light, na.rm = T),
    se_light = sd(light, na.rm = T),
    min_light = min(light, na.rm = T),
    max_light = max(light, na.rm = T)
    )
...
rowgrps <- table(tab[, 1])
tab <- tab[, -c(1:2)]
tab[, c(2:5, 7:10)] <- formtab_fun(tab[, c(2:5, 7:10)], 1, 1, 1)
...
col_heads <- rep(c('$n$', 'Mean', 'St. Err.', 'Min', 'Max'), 2)
...
# seprate zc and light datasets (some light requirement locations were removed if zc > z) 
toplo1 <- select(dat, Longitude, Latitude, z_c_all, seg) %>% 
  na.omit
toplo2 <- select(dat, Longitude, Latitude, light, seg) %>% 
  na.omit
...
p1 <- ggplot(fortify(choc_seg), aes(long, lat)) + 
  ...
  geom_point(data = toplo1, aes(x = Longitude, y = Latitude, 
...
# reorder seg labels
p2 <- ggplot(toplo1, aes(x = seg, y = z_c_all)) + 
...
p3 <- ggplot(fortify(choc_seg), aes(long, lat)) + 
  geom_polygon(colour = 'black', fill = fill_col, aes(group = group)) +
  geom_point(data = toplo2, aes(x = Longitude, y = Latitude, 
...
p4 <- ggplot(toplo2, aes(x = seg, y = light)) + 
  stat_summary(fun.data = f, geom = "boxplot", fill = fill_col) + 
  stat_summary(fun.y = o, geom = "point") +
...
# separate zc and light datasets (some light requirement locations were removed if zc > z) 
sgfac <- rev(c('OTB', 'HB', 'MTB', 'LTB'))
toplo1 <- select(dat, Longitude, Latitude, z_c_all, seg) %>% 
  na.omit %>% 
  mutate(seg = factor(seg, levels = sgfac))
toplo2 <- select(dat, Longitude, Latitude, light, seg) %>% 
  na.omit %>% 
  mutate(seg = factor(seg, levels = sgfac))
