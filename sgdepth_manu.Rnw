...
tab <- tab[, -c(1:2)]
tab[, 4:6] <- formtab_fun(tab[, 4:6])

cap.val<-'Summary of water clarity data and estimated light requirements for all bay segments of the Indian River Lagoon and Tampa Bay.  Water clarity data were obtained from secchi observations in the Florida \\acl{IWR} database for all dates within each  bay.  Minimum year, maximum year, sample size ($n$), and mean values (m) of secchi data by segment are shown. Mean $DOC_{max}$ and estimated \\% light requirements for each segment are based on data in \\cref{fig:light_irl,fig:light_tb}.'
...
col_heads <- c('Min year', 'Max year', '$n$', 'Secchi', '$DOC_{max}$', '\\% light')
