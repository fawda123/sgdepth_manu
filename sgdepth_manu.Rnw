...
<<kz_choc, echo = F, eval = T, cache = T, results = 'hide'>>=
options(digits = 7) 
...
# leg lab
leg_lab <- expression(paste(italic(K [Z]), ' (', m^-1, ')')) 
...
pdf('figs/kz_choc.pdf', height = 4, width = 6, family = 'serif')
p
dev.off()
@
\begin{figure}
...
# proc <- kz_doc(samp_vals, sgpts_2007_choc, choc_seg, radius = 0.1, trace = T)
# dat <- na.omit(proc)
# 
# choc_light <- dat
