...
\end{equation}
A conversion factor is commonly used to estimate the light extinction coefficient from secchi depth $Z_{d}$, such that such that $1.44 = K_{d} \cdot Z_{d}$ \citep{Holmes70}.  Thus, $K_{d}$ is replaced with the conversion factor and the equation is rearranged to describe $DOC_{max}$ as a function of secchi depth $Z_{d}$:
\begin{equation} \label{eqn:sgreg}
DOC_{max} = \frac{-\log\left(0.11\right)}{1.44} \cdot Z_{d}
\end{equation}
\noindent A regression of seagrass depth estimates against secchi measurements is expected to have a slope corresponding to the constant in \cref{eqn:sgreg}, provided that the current approach for estimating maximum \ac{doc} is consistent with previous analyses that have empirically related seagrass depth limits with light requirements.  The geographic coordinates for each secchi measurement were used as locations for estimating $DOC_{max}$ in each segment.  These estimates were compared with the secchi estimates using linear regression forced through the origin.  The slopes of the corresonding regressions were compared with that in \cref{eqn:sgreg} with the assumption that the two would not differ significantly if the method for estimating seagrass \ac{DOC} is robust.  However, the relationship between the depth estimates and secchi measurements may vary depending on the specific radius around each sample point for estimating $DOC_{max}$.  The effect of radius size on the relationship was also explored.         
...
buff_ex_dat <- readRDS('data/buff_ex_dat.rds')
