...
<<all_ests, echo = F, results = 'hide', eval = T>>=
...
<<all_sens, echo = F, results = 'hide', eval = T>>=
...
# proc <- secc_doc(secc_all, sgpts_2010_tb, tb_seg, radius = 0.15, seg_pts_yr = '2010', trace = T)
...
pt_cols <- c('blue', 'lightblue', 'brown')

# legend formatting, z_cmax
scls <- c(2, 9)
scl_vals <- scales::rescale(dat[, c('z_cmax_all'), drop = F], to = scls)
leg_brks <- seq(min(dat$z_cmax_all, na.rm =  T), max(dat$z_cmax_all, na.rm = T), length = 4)
leg_labs <- form_fun(leg_brks, 2, 2, 2)
...
p1 <- ggplot(fortify(tb_seg), aes(long, lat)) + 
  ...
  geom_point(data = dat, aes(x = Longitude, y = Latitude, 
    colour = z_cmax_all, size = z_cmax_all), fill = 'black', alpha = 0.8) +
...
p2 <- ggplot(dat, aes(x = seg, y = z_cmax_all)) + 
