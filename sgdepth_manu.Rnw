...
Seagrass \ac{doc} estimates were obtained for \Sexpr{nrow(choc_light)} locations in Choctawhatchee Bay, \Sexpr{nrow(tb_light)} locations in Tampa Bay, and \Sexpr{nrow(irl_light)} locations in the Indian River Lagoon (\Cref{tab:light_summ,fig:light_choc,fig:light_tb,fig:light_irl}).  Mean $Z_{c,\,max}$ for each bay was \Sexpr{zcall_means$choc}, \Sexpr{zcall_means$tb}, and \Sexpr{zcall_means$irl} m for Choctawhatchee Bay, Tampa Bay, and Indian River Lagoon, respectively, with all values being significantly different beetween bays (ANOVA, $F=$ \Sexpr{zcall_aov['Fval']}, $df =$ \Sexpr{zcall_aov['df1']}, \Sexpr{zcall_aov['df2']}, $p < 0.001$, followed by Tukey multiple comparison, $p<0.001$ for all).  Generally, spatial variation in $Z_{c,\,max}$ followed variation in light requirements for broad spatial scales with more seaward segments within each bay having lower light requirements.  Mean light requirements were significantly different between all bays (ANOVA, $F=$ \Sexpr{aov_btw['Fval']}, $df =$ \Sexpr{aov_btw['df1']}, \Sexpr{aov_btw['df2']}, $p < 0.001$, Tukey $p<0.001$ for all), with a mean requirement of \Sexpr{mean(choc$light)}\% for Choctawhatchee Bay, \Sexpr{mean(tb$light)}\% for Tampa Bay, and \Sexpr{mean(irl$light)}\% for Indian River Lagoon.  Significant differences in light requirements between segments within each bay varied.  All segments in Tampa Bay were significantly different except Middle Tampa Bay and Hillsborough Bay.  Within each bay, light requirements were significantly different between segments (ANOVA, $F =$ \Sexpr{form_fun(wtn_choc['Fval'], 1, 1, 1)}, $df =$ \Sexpr{wtn_choc['df1']}, \Sexpr{wtn_choc['df2']}, $p < 0.001$ for Choctawhatchee Bay, $F =$ \Sexpr{form_fun(wtn_tb['Fval'], 1, 1, 1)}, $df =$ \Sexpr{wtn_tb['df1']}, \Sexpr{wtn_tb['df2']}, $p < 0.001$ for Tampa Bay, $F =$ \Sexpr{form_fun(wtn_irl['Fval'], 1, 1, 1)}, $df =$ \Sexpr{wtn_irl['df1']}, \Sexpr{wtn_irl['df2']}, $p < 0.001$ for Indian River Lagoon).  Post-hoc evaluation of all pair-wise comparisons of mean light requirements indicated that significant differences varied between segments within each bay.  Significant differences were observed between all segments in Choctawhatchee Bay, except the central and western segments (\cref{fig:light_choc}).  Similarly, significant differences in Tampa Bay were observed between all segments, except Middle Tampa Bay and Old Tampa Bay (\cref{fig:light_tb}).  Finally, for the Indian River Lagoon, significant differences were observed between the Lower Central Indian River Lagoon and the Upper ($p<0.001$) and Lower Mosquito Lagoons ($p = $ \Sexpr{form_fun(tuk_irl['LML-LCIRL'], 3, 3, 3)}),  the Lower Indian River Lagoon and the Upper ($p<0.001$) and Lower Mosquito Lagoons ($p =$ \Sexpr{form_fun(tuk_irl['LML-LIRL'], 3, 3, 3)}), and the Upper Central Indian River and the Upper Mosquito Lagoon ($p = $\Sexpr{form_fun(tuk_irl['UML-UCIRL'], 3, 3, 3)}) (\cref{fig:light_irl}).
