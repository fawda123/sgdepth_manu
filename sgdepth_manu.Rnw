...
sats_melt <- reshape2::melt(choc_sats[['sats_all']], id.var = c('lat', 'lon'))
...
# proc <- kd_doc(samp_vals, sgpts_2007_choc, choc_seg, radius = 0.1, trace = T)
# dat <- na.omit(proc)
# 
# choc_light <- dat
...
p2 <- ggplot(dat, aes(x = seg, y = z_c_all)) + 
  geom_boxplot(fill = fill_col, width = 0.75) + 
