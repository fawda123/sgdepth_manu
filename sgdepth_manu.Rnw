...
p3 <- ggplot(seg, aes(long, lat)) + 
  ...
  scalebar(location = 'bottomleft', y.min = 29.42, y.max = 29.5, x.min = -83.63, x.max = -83.45, 
...
\begin{figure}
\centering
\includegraphics[width = 0.7\textwidth]{figs/Fig8.pdf}
...
<<tb_ts, echo = F, eval = T, cache = T, results = 'hide'>>=
...
# boxplots
p1 <- ggplot(sec, aes(x = factor(yr), y = light, fill = Segment)) +
  ...
  scale_y_continuous('% light requirements')
...
# labels
labs <- filter(summs, yr %in% c('1988', '2010'))
ylab <- expression(italic(Z ['c, med']), '(m)')
xlab <- expression(paste(italic(K [d]), ' (', m^-1, ')')) 

# time paths
p2 <- ggplot(summs, aes(x = kd, y = z_c_all, colour = Segment)) +
  geom_path() +
  geom_point(pch = 21, aes(fill = Segment), colour = 'black', size = 2.5) + 
  geom_label(data = labs, aes(label = yr, fill = Segment), 
    colour = 'black', size = 3.5, label.padding = unit(0.1, "lines")) +
...
pdf('figs/FigS1.pdf', height = 8, width = 8, family = 'serif')
grid.arrange(pA, p1leg, pB, blankPanel, pC, p3leg, pD, blankPanel, 
  ncol = 2, nrow = 4, 
  widths = c(1, 0.2), heights = c(1, 0.7, 1, 0.7))
dev.off()
...
pdf('figs/FigS2.pdf', height = 7.5, width = 7, family = 'serif')
grid.arrange(p1, p1leg, p2, p3, p3leg, p4, ncol = 3, nrow = 2, 
  widths = c(1, 0.3, 0.75))
dev.off()
@
...
pdf('figs/FigS3.pdf', height = 8.5, width = 6.5, family = 'serif')
grid.arrange(p1, p1leg, p2, p3, p3leg, p4, ncol = 3, nrow = 2, 
  widths = c(1, 0.3, 0.75))
dev.off()
...
pdf('figs/FigS4.pdf', height = 5.5, width = 5, family = 'serif')
p1
dev.off()
@
\begin{figure}
\centering
\includegraphics[width = 0.7\textwidth]{figs/FigS4.pdf}
