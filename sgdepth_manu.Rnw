...
p <- ggplot(data = seg_plo) +
  ...
  coord_equal() +
  facet_wrap(~ variable) + 
  scale_colour_gradientn('Clarity (m)', colours = cols) +
  scale_fill_gradientn('Clarity (m)', colours = cols) +
...
# proc <- secc_doc(samp_vals, sgpts_2010_tb, tb_seg, radius = 0.1, seg_pts_yr = '2010', trace = T)
