...
zm1 <- lme(z_c_all ~ 0 + bay, random = ~ 1 | bay,
  correlation = corGaus(form = ~ Latitude + Longitude | bay, nugget = FALSE), 
  data = all_light)
zm2 <- lme(z_c_all ~ 0 + bay, random = ~ 1 | bay,
...
lm1 <- lme(light ~ 0 + bay, random = ~ 1 | bay,
  correlation = corGaus(form = ~ Latitude + Longitude | bay, nugget = FALSE), 
  data = all_light)
lm2 <- lme(light ~ 0 + bay, random = ~ 1 | bay,
  correlation = corGaus(form = ~ Latitude + Longitude | bay, nugget = TRUE), 
  data = all_light)
...
<<>>=
# boxplots
ggplot(all_light, aes(x = bay, y = light)) +
  geom_boxplot() + 
  theme_minimal()
@
...
stargazer(znl, zm1, zm2, lnl, lm1, lm2,
  title = 'Inter-bay differences for median depth of colonization and light requirements. Null models do not include a grouped correlation structure. Gaussian models with and without nuggets for the spatial correlation structures are also shown.',
  column.labels = c('null', 'Gaus', 'Gaus, nug', 'null', 'Gaus', 'Gaus, nug'), 
  model.numbers = F
  )
@
...
summary(glht(zm2, linfct = mcp(bay = 'Tukey')))
summary(glht(lm1, linfct = mcp(bay = 'Tukey')))
summary(glht(lm2, linfct = mcp(bay = 'Tukey')))
@
